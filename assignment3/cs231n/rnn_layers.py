...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    ##############################################################################
    next_z = x.dot(Wx) + prev_h.dot(Wh) + b
    next_h = np.tanh(next_z)
    cache = (x, Wx, prev_h, Wh, next_z)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    df = prev_c*dnext_c; 
    dprev_c = f*dnext_c
    da = np.hstack((sigmoid(a[:, :H])*(1-sigmoid(a[:, :H]))*di
                   , sigmoid(a[:, H:2*H])*(1-sigmoid(a[:, H:2*H]))*df
                   , sigmoid(a[:, 2*H:3*H])*(1-sigmoid(a[:, 2*H:3*H]))*do
                   , (1-np.tanh(a[:, 3*H:])**2)*dg))
    db = np.sum(da, axis=0)
    dx = da.dot(Wx.T)
    dprev_h = da.dot(Wh.T)
    dWx = x.T.dot(da)
